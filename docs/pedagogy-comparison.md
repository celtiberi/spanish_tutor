...
4. **Persist student state across sessions and schedule spaced returns** (app + policy change): failed items reappear the next day, then at expanding calendar intervals (~3 days, ~7 days); interleave *ser/estar/tener* items rather than blocked unit mastery. The state plumbing already exists; it needs cross-session storage and a due-date field. *(Implemented in pack v0.4 / policy v0.2 with this calendar algorithm.)*
