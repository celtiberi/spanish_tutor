...
- **PLAN turns** (session open, or after `<replan/>`) carry the FULL context: PEDAGOGY.md verbatim, full pack, full sheet, full history; the model writes its own private `<plan>` block.
- **ROUND turns** carry the model's OWN plan + full sheet + session facts + due data + the last `ROUND_HISTORY_MESSAGES` (=12, versioned in `tutor/session_plan.py`) messages; no pack, no pedagogy file. The model may revise its plan any turn and escapes to full context with `<replan/>`.
