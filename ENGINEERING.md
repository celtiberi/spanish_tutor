...
- **PLAN turns** (session open, or after `<replan/>`) carry the FULL context: PEDAGOGY.md verbatim, the full character sheet, full history; the model writes its own private `<plan>` block. **Same-day addendum (USER: "the character sheet IS the course pack"):** the prose course pack was DELETED — the sheet now carries the curriculum itself (`curriculum_targets_not_yet_touched` from the association table, `curriculum_scope` deferred/out-of-scope/recognition-only law, the pack's frozen form inventories as grammar targets, its misconception catalog as diagnosis vocabulary with empty detect — the model diagnoses, code records). Curriculum DATA (association table, teach-asset sidecar, scenes) lives in `curriculum/spanish_a1/`. The B0 "brief" arm was deleted with the pack it fed on.
