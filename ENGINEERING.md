...
**Fact-surface clause (2026-08-05, ⬛ Grok ACCEPT_WITH_AMENDS, docs/archive/reviews/pre-grading-20260805.md):** code MAY inject **offline dictionary facts** about the current learner utterance into the turn task (token membership/frequency against frozen inventories, multi-candidate neighbors for invalid tokens, verb-form identification against the conjugation table). This is record-keeper surface co-located with the sheet data — NOT a reopening of hard_observations. Forbidden forever in that block: error-pattern labels, probe/mode signals, band or next-teach hints, any singleton "nearest"/repair target (anchoring: a privileged repair string is a recast prime — killed in review). The block ships only behind the pre-registered A/B in the review doc; if it fails to move the garble-credit rate it is DELETED, not kept (§4.6). **Outcome (2026-08-05, same day): the A/B ran and Arm B (neutral cands) FAILED the bar — sam garble-credit g: control 0.471, cands 0.483 (no movement), and the forbidden Arm C nearest-anchor ablation was the only arm that moved it (0.250) — confirming the review's anchoring model in both directions: neutral facts were decoration, the anchor works because it is an anchor. Code paths deleted per pre-registration. Any revival is a NEW round that must solve salience-without-priming; this clause stays as the constitutional record.**
...
**AMENDMENT 2026-08-06 (P1 history bound — ⬛ Grok-countersigned design, docs/analysis-multi-model-context.md converged; doc-sync included).** (1) *Correction:* the 12-message round window above was superseded 2026-08-04 by the plan-cycle APPEND-ONLY suffix (cache arm — `stage_prompt_build`); this paragraph is the law's record catching up with shipped code. (2) *The bound:* live sessions measured p50 19k / p90 24k input tokens with the suffix unbounded. Rounds may now carry, ONCE the cycle suffix exceeds `ROUND_SUMMARY_THRESHOLD_MSGS` (versioned in `tutor/session_plan.py`): a facts-only ROLLING SUMMARY of the older suffix — authored ASYNC by a cheap model off the reply path — which MUST embed verbatim learner-error spans (the "exact wording three turns ago" risk, countersigned mandatory), plus the last `ROUND_SUMMARY_KEEP_EXCHANGES` raw exchanges tail-aligned. The full transcript stays on disk; the SHEET remains the sole ability authority — a summary never grades, and any ability claim in summary text is void. Summarizer failure ⇒ full suffix ships as before (degrade open, visibly). The slice is `truncation-ok`-annotated and the constants are versioned; silent slices remain banned. Kill bar (pre-registered): persona-gate repair/fixation regressions attributable to lost wording ⇒ revert the bound, keep the law.
...
6. **The output gate is a PLUMBING auditor, and it SURFACES — it never hides (AMENDED 2026-08-03, S11 USER ruling, docs/reviews-full-code-audit-20260803.md: "Why are you making gates for this?"; NO-HIDE unchanged from 2026-08-01, commit 898af6e):** the gate checks every reply for exactly two faults code can actually judge — `gate:truncated` (provider cut the reply) and `gate:sheet_leak` (internal JSON/tool talk in learner-visible text) — and on failure the RAW reply ships with `gate_fail` + a visible diagnosis: no repair rewrites, no part stripping, no blank holds, no content scrub. Every teaching-quality check the gate used to run (english_wall, unscaffolded_new_item, probe_loop, regloss, cluster_veto, the pedagogy-contract shapes) is DELETED from the runtime (§4.6) and lives ONLY as eval test cases over AI-student transcripts (evals/student_checks.py + the blind rubric) — evals judge whether pedagogy + prompts are working; the teaching rules themselves stay in PEDAGOGY §2 and still ship to the model. first_seen exposure bookkeeping survives in the gate (record-keeping, not judgment). *(Superseded by this amendment: the 2026-08-01 fault list naming the teaching checks as runtime audit labels, and the 2026-07-30 repair-ladder text before it — trails in docs/archive/reviews/.)* Checker budget direction unchanged as PEDAGOGY-side teaching law: ≤1 comprehension check per 3 turns; never a meaning quiz on sheet-known material; due items return as §2.4 natural elicits only.
...
| §2.2 naked items | evals/student_checks.py check_cluster_intro (HARD) + check_exposure_advisories (WARN) — *amended 2026-08-03, S11: was gate:unscaffolded_new_item + gate:cluster_veto runtime faults* | countersign rounds on table |
...
| §6 no-hide gate | gate_fail surface + visible diagnosis (tests/test_gate_floor.py TestGateNoHide: no repair/strip/hold events, raw ships); session still-fail counter — *amended 2026-08-03, S11: fault vocabulary is truncated + sheet_leak only* | USER-enacted 2026-08-01 (commit 898af6e); prior ladder trail archived |
...
| §2.3 walls/regloss | evals/student_checks.py check_english_wall + check_exposure_advisories re-gloss arm (WARN) — *amended 2026-08-03, S11: was gate:english_wall + gate:regloss runtime faults* | blind rubric |
