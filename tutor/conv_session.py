...
OPEN_HARNESS = (
    "(harness) Open a warm conversational Spanish session. "
    ...
    "(optional short <acknowledge>). "
    "Call update_character_sheet only if you already know something new "
    "about the learner (usually skip on session open)."
)
        ...
        *,
        ...
        if active:
            ...
            err_line = (
                ...
                f"Recast if they hit it again; weave a short correct use into continue.\n"
            )
        harness = (
            f"<harness_context>\n"
            f"now={now_iso()}\n"
            f"Character sheet is your model of this student — adapt to it.\n"
            f"next_best: {json.dumps(self.sheet.get('next_best'), ensure_ascii=False)}\n"
            f"{err_line}"
            f"{scaffold_line}\n"
            f"{mode_line}\n"
            f"OUTPUT: use <tutor> structured parts "
            f"(acknowledge / recast / explain / continue). "
